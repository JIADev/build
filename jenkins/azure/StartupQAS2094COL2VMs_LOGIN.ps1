...
$WebvmrgName = "QAS-2094-COL2webrg"
$SQLSSRvmrgName = "QAS-2094-COL2sqlssrrg"

write-host "Retrieving all VM's in Resource Group '"$SQLSSRvmrgName "'"
$vms = Get-AzureRMVM -ResourceGroupName $SQLSSRvmrgName
$vmrunninglist = @()


Foreach ($vm in $vms)
{
	$vmstatus = Get-AzureRMVM -ResourceGroupName $SQLSSRvmrgName -name $vm.name -Status
...
if ($Powerstate -eq 'Deallocated')
{
	write-host "Starting VM "$vmrunninglist " in Resource Group "$SQLSSRvmrgName ""
	$vmrunninglist | Invoke-Parallel -ImportVariables -NoCloseOnTimeout -ScriptBlock {
		Start-AzureRMVM -ResourceGroupName $SQLSSRvmrgName -Name $_ -Verbose
	}
...
write-host "Retrieving all VM's in Resource Group '"$WebvmrgName "'"
$vms = Get-AzureRMVM -ResourceGroupName $WebvmrgName
$vmrunninglist = @()


Foreach ($vm in $vms)
{
	$vmstatus = Get-AzureRMVM -ResourceGroupName $WebvmrgName -name $vm.name -Status
	$PowerState = (get-culture).TextInfo.ToTitleCase(($vmstatus.statuses)[1].code.split("/")[1])
...
if ($Powerstate -eq 'Deallocated')
{
	write-host "Starting VMs "$vmrunninglist " in Resource Group "$WebvmrgName ""
	$vmrunninglist | Invoke-Parallel -ImportVariables -NoCloseOnTimeout -ScriptBlock {
		Start-AzureRMVM -ResourceGroupName $WebvmrgName -Name $_ -Verbose
	}
