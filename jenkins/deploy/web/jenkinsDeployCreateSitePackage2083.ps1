...
else
{
	...
	if ((Test-Path -Path $releasePath -PathType container))
	{
		...
		gci -Path "$releasePath" -Recurse | Where-Object { $_.FullName -like "*MSDeploy\*.zip" -or $_.FullName -like "*version.txt" } | % { Copy-Item -Path $_.FullName -Destination "$releasePath\Site" -Force }
		...
		$portalZips = gci -Path $siteDir | where { $_ -like '*.zip' } | select -ExpandProperty FullName
		foreach ($portal in $portalZips)
		{
			...
			$sitePath = $siteDir + "\" + $portalName.Substring(0, $portalName.LastIndexOf('.'))
			Expand-Archive -Path "$portal" -DestinationPath "$sitePath-temp" -Force
		}
		
		gci $siteDir -Recurse -Include *.zip | Remove-Item -Force -Recurse
		
		#Copy site files to Site folder. *Maybe add the correct web.config*
		Write-Host "Copy site files to Site folder."
		$sites = gci $siteDir | where { $_.PSIsContainer -eq $true } | select -ExpandProperty FullName
		...
		gci $siteDir | where { ($_ -like '*-temp') -and $_.PSIsContainer -eq $true } | Remove-Item -Force -Recurse
		...
		$zipFileSite | Out-File -FilePath $versionPath -Force
