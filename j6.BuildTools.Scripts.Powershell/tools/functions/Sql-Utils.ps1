...
function restore-database {
	...
	#$restore.DatabaseFiles.Add((join-path $foldexecute-reader ($database + "_log.ldf")))
...
function get-theme {
	...
	$ret = (execute-reader ("select theme from personal.personalsite where code='{0}'" -f $site))
	return $ret.theme
}
...
function set-theme {
	...
	$null = execute-nonquery ("update personal.personalsite set theme='{0}' where code = '{1}'" -f $theme, $site)
	get-theme $site
}
