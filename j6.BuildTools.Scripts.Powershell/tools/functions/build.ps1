...
function clear-portalusers {
	execute-nonquery "delete from security.applicationuser"
	execute-nonquery "delete from security.portaluser"
}

function create-portalusers {
	execute-nonquery "INSERT INTO [Security].[PortalUser]
           ([Code]
           ,[Account]
           ,[AccountCode]
           ,[Password]
	...
	execute-nonquery "INSERT INTO [Security].[ApplicationUser]
           ([PortalUser]
           ,[Application])
     select p.id, a.id
...
function get-columns {
	param([string]$table)
	execute-reader "select COLUMN_NAME as [NAME], ORDINAL_POSITION as [POS], COLUMN_DEFAULT as [DEFAULT], IS_NULLABLE as [NULL?],
...
function get-table {
	param([string] $table)
	execute-reader "select * from information_schema.tables where upper(table_name) like upper('$table')"
}
...
function recreate-database {
	...
	execute-nonquery "DROP DATABASE [$db]"
	execute-nonquery "CREATE DATABASE [$db]"
	set-database -server $server -db $db -user $user -pass $pass
