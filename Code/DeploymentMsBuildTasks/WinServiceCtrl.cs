	...
	class Program
	{
		...
		private static int Main(string[] args)
		{
			...
			try
			{
				...
				var msBuildCommand = new WinServiceControl
				{
					ServiceName = new [] { args[0] },
					ComputerName = ValidOperations.Contains(args[1], StringComparer.InvariantCultureIgnoreCase)
								   ? Environment.MachineName
								   : args[1],
					RequestedOperation = operation
				};

				return msBuildCommand.Run();
			}
			catch (Exception ex)
			{
